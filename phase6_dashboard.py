...
fig_speed.write_html("speed_meter.html")

# Altitude chart
if not df.empty:
    fig_alt = px.line(df, x="callsign", y="baro_altitude", title="Flight Altitudes over Pakistan")
    fig_alt.write_html("altitude_chart.html")

# Data table
if not df.empty:
    df.to_html("flight_data_table.html", index=False)

print("Dashboard components saved: pakistan_flights.html, speed_meter.html, altitude_chart.html, flight_data_table.html")
